...
而且默认视图会主动误导你：主页仓库列表默认按 `Last updated` 排序，谁最后被推过一次谁就在最上面。我实测时的情况是——一个随手 fork 的第三方工具，把我全部主力项目压在了下面。
...
| 错误做法 | 后果 | 正确做法 |
|---|---|---|
| 用"默认分支上本人提交数为 0"判定某个 fork 没用 | 误判：那个 fork 其实带着 3 个已提交上游的 PR，只是都开在独立分支上，默认分支当然和上游一致 | 判 fork 要同时看 、、以及本地工作目录的未推分支 |
| 按  排活跃度 | 批量导入/镜像会把一堆仓库刷成同一天，排出的是"哪天搬运过" | 活跃度靠人工判据（30 天内打开过几次），时间戳只当参考 |
